...
negamax(J, Etat, P, Pmax, [rien, Val]) :-
	...
	heuristique(J,Etat,Val), !.
negamax(J, Etat, P, Pmax, [[C1,C2], Val]) :-
	P < Pmax,
	successeurs(J,Etat,Succ),
	loop_negamax(J,P,Pmax,Succ,Liste_Couples),
	meilleur(Liste_Couples, [[C1,C2],V1]),
	Val is -V1.
...
A FAIRE : commenter chaque litteral de la 2eme clause de loop_negamax/5,
	...
	loop_negamax(J,P,Pmax,Succ,Reste_Couples) : boucle sur le couple suivant de [Coup, Si_suiv] et [Coup,Val_suiv], la suite de la liste
	adversaire(J,A) : prend l'adversaire A du joueur J
	Pnew is P+1 : On incremente Pnew
	negamax(A,Suiv,Pnew,Pmax, [_,Vsuiv]) : on applique la clause negamax sur l'adversaire du joueur courant, la situation suivante, la nouvelle valeur de profondeur, Pmax,
											peu importe le coup, on retourne la valeur suivante dont a besoin loop_negamax
	*/
...
meilleur([[[C1,C2],V]],[[C1,C2],V]).
meilleur([[[C1,C2],V]|Rest], [[Cm1,Cm2],Vm]) :-
	meilleur(Rest,[[CmRest1,CmRest2],VmRest]),
	(V < VmRest -> (Vm = V,Cm1 = C1, Cm2 = C2); (Vm = VmRest,Cm1 = CmRest1, Cm2 = CmRest2)).
